...
def gogo(years=['2017'], pages=['1', '2', '3', '4']):
    samples = []
        ...
        for page in pages:
            ...
            for container in mv_containers:
                ...
                try:
                    ...
                    if container.find('div', class_='ratings-metascore') is not None:
                        ...
                        # datapoint['year'] = year[1:-1]

                        # Scrape the IMDB rating
                        # imdb = container.strong.text
                        # imdb_ratings.append(imdb)
                        # datapoint['IMDB rating'] = imdb
                        # Scrape the Metascore
                        # m_score = container.find('span', class_='metascore').text.strip()
                        # datapoint['Metascore'] = m_score

                        # Scrape the number of votes
                        # vote = container.find('span', attrs={'name': 'nv'})['data-value']
                        # datapoint['vote'] = vote
                        # serach for certificate:
                        # span = container.find('span' ,class_="certificate")
                        # datapoint['certificate'] = span.text
            ...
            for feature_dict in samples:
                for key in feature_dict.keys():
                    keys.add(key)
            keys = list(keys)
            with open('model1_' + year_url + page, 'w') as output_file:
