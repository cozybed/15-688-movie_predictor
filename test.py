...
def analysis(y_p, y_t):
    ...
    for i,val in enumerate(diff):
        if val != 0:
            print(y_p[i], y_t[i], val, gross_df[1800+i]) 
...
clf_lg.fit(X[:1800,:], y[:1800])
y_p = clf_lg.predict(X[1800:,:])
print("Validation accuracy:", np.mean(y_p==y[1800:]))

#analysis(y_p, y[1800:])
print(wrong_pred_distribution(y_p, y[1800:]))
print(wrong_true_distribution(y_p, y[1800:]))


#################
clf = SVC()
clf.fit(X[:1800,:], y[:1800])
y_p = clf.predict(X[1800:,:])
'''
...
print(wrong_pred_distribution(y_p, y[1800:]))
print(wrong_true_distribution(y_p, y[1800:]))
indice = get_wrong_index(y_p, y[1800:])
...
clf_tree.fit(X[:1800,:], y[:1800])
y_p = clf_tree.predict(X[1800:,:])
print("Validation accuracy:", np.mean(y_p==y[1800:]))
